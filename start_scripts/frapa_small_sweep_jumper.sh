ENV_ID="procgen-jumper"
WANDB_PROJECT_NAME="jumper_frapa_results_and_success"
...
for x in "${X_VALUES[@]}"; do
    ...
    #--max_val_only=1 --proc_num_levels=100 --track --wandb_project_name="$WANDB_PROJECT_NAME" --easy=1 --use_monochrome=0 \
    #--reward_limit=7.5
    ...
    #     10
    python3 "$(pwd)/methods/frapa_ppo.py" --seed="$x" --env_id="$ENV_ID" \
    --number_hs=10 --min_similarity_score=0.98 --max_dict_in_compress=1 \
    --max_val_only=1 --proc_num_levels=100 --track --wandb_project_name="$WANDB_PROJECT_NAME" --easy=1 --use_monochrome=0 \
    --reward_limit=7.5 
